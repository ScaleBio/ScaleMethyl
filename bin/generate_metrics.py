...
def accept_args() -> argparse.Namespace:
    ...
    parser.add_argument("--top_cell_percentage", default=99.0, type=float,
                        help="Percentage of cells over min_reads to use as 'robust max'")
    parser.add_argument("--min_cell_ratio", default=200.0, type=float,
                        help="Ratio between transcript counts of top cells and the lower cell threshold")
    parser.add_argument("--min_reads", default=1000.0, type=float,
                        help="Minimum counts to consider a barcode as a potential cell")
    parser.add_argument("--min_uniq_total", default=0.0, type=float,
                        help="Minimum uniqCounts/Total percentage cutoff")
    parser.add_argument("--max_uniq", type=float,
                        help="Maximum unique counts cutoff (optional; default = max of tsv)")
    parser.add_argument("--max_uniq_total", default=100.0, type=float,
...
def construct_all_cells(max_uniq: int, max_uniq_total: int, sample_name: str, min_uniq_total: int, cell_stats: Path,
                        out_dir: Path, top_cell_percentage: float, min_cell_ratio: float, min_reads: float, threshold: float,
                        cell_info: Path, library_structure_json: Path):
    """
    ...
    if len(all_cells) == 0:
        ...
        sys.exit(1)
    all_cells.rename(columns={'BC': 'cell_id', 'total': 'total_reads', 'uniq': 'unique_reads', 'passing': 'passing_reads'}, inplace=True)
    ...
    if not threshold or threshold==0:
        ...
        if expected_cells == 0:
            ...
            calc_threshold = float(np.percentile(all_cells["unique_reads"][:expected_cells], (top_cell_percentage))/min_cell_ratio)
    ...
    all_cells.loc[
        (all_cells['percent'] >= min_uniq_total) & (all_cells['unique_reads'] >= threshold) &
        (all_cells['percent'] <= max_uniq_total) & (all_cells['unique_reads'] <= max_uniq),
        "pass"
    ] = "pass"

    for barcode_order, barcode_info in enumerate(json.load(open(library_structure_json))['barcodes']):
        barcodes_df = pd.read_csv(library_structure_json.parent / barcode_info['sequences'], names=[f"{barcode_info['name']}", f"{barcode_info['name']}_well"], sep="\t")
        all_cells[f"{barcode_info['name']}"] = all_cells["cell_id"].str.split("+").str[barcode_order]
        all_cells = pd.merge(all_cells, barcodes_df, left_on=f"{barcode_info['name']}", right_on=f"{barcode_info['name']}", how="left")
    ...
    print(f"Number of passing rows in all_cells: {all_cells[all_cells['pass']=='pass'].shape[0]}", file=sys.stderr)    
...
def main():
    ...
    construct_all_cells(
        args.max_uniq, args.max_uniq_total, args.sample_name, args.min_uniq_total, args.cell_stats, args.out_dir,
        args.top_cell_percentage, args.min_cell_ratio, args.min_reads, args.threshold, args.cell_info, args.library_structure_json
    )
