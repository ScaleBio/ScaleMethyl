...
def construct_all_cells(
    ...
    all_cells["percent"] = round(
        (all_cells["unique_reads"] / all_cells["total_reads"] * 100), 3
    )
    all_cells["mito_reads"] = round(
        (all_cells["MitoReads"] / all_cells["total_reads"] * 100), 3
    )
    ...
    if not threshold or threshold == 0:
        ...
        if expected_cells == 0:
            ...
            calc_threshold = float(
                np.percentile(
                    all_cells["unique_reads"][:expected_cells], (top_cell_percentage)
                )
                / min_cell_ratio
            )
    ...
    for barcode_order, barcode_info in enumerate(
        json.load(open(library_structure_json))["barcodes"]
    ):
        ...
        all_cells[f"{barcode_info['name']}"] = (
            all_cells["cell_id"].str.split("+").str[barcode_order]
        )
        all_cells = pd.merge(
            all_cells,
            barcodes_df,
    ...
    all_cells["ch_high_reads_percent"] = round(
        (all_cells["ch_high_reads"] / all_cells["total_reads"] * 100), 3
    )
