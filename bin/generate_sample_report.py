...
def accept_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mapping_stats", type=Path, help="Path to concatenated bsbolt log file"
    )
    parser.add_argument(
        "--trimming_stats", type=Path, help="Path to concatenated trim_galore log file"
    )
    parser.add_argument(
        "--fragment_hist", type=Path, help="Path to concatenated fragment_hist.tsv file"
    )
    parser.add_argument(
        "--dedup_stats", type=Path, help="Path to concatenated dedup_stats.tsv file"
    )
    parser.add_argument(
        "--out_dir",
        default=".",
    ...
    parser.add_argument(
        "--sample_name", type=str, required=True, help="Identifier for this sample"
    )
    parser.add_argument(
        "--tss_enrich",
        type=str,
    ...
    parser.add_argument(
        "--sample_barcodes", type=str, help="list of all sample barcodes in one string"
    )
...
def main():
    ...
    if args.mapping_stats and args.trimming_stats:
        ...
        shutil.copy(
            args.mapping_stats, f"{args.sample_name}/csv/{args.mapping_stats.name}"
        )
        shutil.copy(
            args.trimming_stats, f"{args.sample_name}/csv/{args.trimming_stats.name}"
        )
    else:
        mapping_stats = False
    ...
    if not concat_tss_enrich_df.empty:
        if "tss_enrich" not in all_cells.columns:
            all_cells = all_cells.merge(
                concat_tss_enrich_df, on=["cell_id"], how="left"
            )
        print(
            f"Number of NaN values in tss enrichment column: {all_cells['tss_enrich'].isna().sum()}"
        )
        print(
    ...
    datapane_obj = BuildDatapane(
        args.sample_name, writeDir, all_cells, False, fragment_df
    )
    ...
    dp_list_summary.append(
        datapane_obj.build_sample_information_table(sample_information)
    )
    dp_list_summary.append(
        datapane_obj.build_summary_stats_table(mapping_stats, trimming_stats)
    )
    if not met_passing.empty:
        dp_list_summary.append(datapane_obj.construct_passing_cell_stats())
    dp_page.append(
        dp.Page(dp.Group(blocks=dp_list_summary, columns=2), title="Summary")
    )
    ...
    dp_page.append(
        dp.Page(dp.Group(blocks=dp_list_read_qc, columns=2), title="Cell Stats")
    )
