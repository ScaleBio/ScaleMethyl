...
def write_batch(
    ...
    batch = (
        ...
        .with_columns(
            pl.when(pl.col("xb").str.contains(r"[XYZ]"))
            .then(1)
            .otherwise(0)
            .alias("methylated")
            .cast(pl.UInt32),
            ...
            .alias("unmethylated")
            .cast(pl.UInt32),
            pl.when(pl.col("xb").str.contains(r"[xX]"))
            .then(pl.lit("CG"))
            .otherwise(pl.lit("CH"))
            .alias("context"),
        )
    ...
    with pysam.AlignmentFile(bam, "rb") as samfile:
        ...
        with pq.ParquetWriter(f"{sample}.RNAME_{chr}.parquet", pa_schema) as writer:
            ...
            for read in reads:
                ...
                pos_offset = np.array(
                    [1 if not char.isnumeric() else int(char) for char in xb]
                ).cumsum()
                ...
                for idx in calls_idx:
                    ...
                    calls["strand"].append(
                        {"C": "-", "W": "+"}[
                            read.get_tag("YS")[0] if read.has_tag("YS") else "W"
                        ]
...
def generate_cell_info_db_prompt(sample, contexts):
    ...
    join_clauses = [
        f"\n    FULL OUTER JOIN '{sample2}_cellInfo_{context}.parquet' USING (CellID)"
...
def met_extract(bam: Path, sample: str, threshold: float, nproc: int, contexts: list):
    """
    ...
    with pysam.AlignmentFile(bam, "rb") as samfile:
        ...
        chrs = [
            stat.contig for stat in samfile.get_index_statistics() if stat.total > 0
        ]

    with Pool(nproc) as p:
        chr_high_ch_reads = p.starmap(
            write_chr_parquet, zip(repeat(bam), chrs, repeat(sample), repeat(threshold))
        )
    ...
    if high_ch_reads:
        high_ch_df = pl.DataFrame(
            {"CellID": high_ch_reads.keys(), "CH_high": high_ch_reads.values()}
        )
    ...
    cell_info_df.write_csv(
        f"{sample}.cellInfo.txt", separator="\t", include_header=True
    )


def main():
    parser = argparse.ArgumentParser(
        "Create methylation calls parquet files from dedup bam file"
    )
    parser.add_argument(
        "bam", type=Path, help="Coordinate-sorted dedup bam file aligned by bsbolt"
    )
