...
Since you will need to provide a bcl-convert samplesheet for generating fastq files from a runfolder, we have example samplesheets you can refer to for building your own. See [samplesheet_pcr_split_largekit.csv](examples/samplesheet_pcr_split_largekit.csv) and [samplesheet_pcr_split_smallkit.csv](examples/samplesheet_pcr_split_smallkit.csv) for example bcl-convert samplesheets split by PCR.  
...
To generate the bcl-convert samplesheet outside of the workflow, this can be done by running the `bclConvertSheet.py` script.
